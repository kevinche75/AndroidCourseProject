...
//            mMagicDefense *= 0.85;
//        }
        String string = CharacterCreator.Perk.BERSERK.name();
        boolean isBerserk = false;
        if(mPerks.containsKey("Berserk") && mPerks.get("Berserk")!=null){
            isBerserk = mPerks.get("Berserk");
        }
        if (isBerserk) {
            mAttack *= 1.1;
            mPhysicalDefense *= 0.85;
        ...
        if(mPerks.containsKey("Calm")&& mPerks.get("Calm")!=null)
        isCalm = mPerks.get("Calm");
        if (isCalm) {
            mAttack *= 0.85;
        ...
        if(mPerks.containsKey("Lightweight")&& mPerks.get("Lightweight")!=null)
            isLightWeight = mPerks.get("Lightweight");
        if (isLightWeight) {
            mDodge *= 1.1;
            mPhysicalDefense *= 0.85;
        }

        boolean isHeavyArmored = false;
        if(mPerks.containsKey("Heavyarmored")&& mPerks.get("Heavyarmored")!=null)
        isHeavyArmored = mPerks.get("Heavyarmored");
        if (isHeavyArmored) {
            mPhysicalDefense *= 1.1;
            mDodge *= 0.85;
        }

        boolean isMeditations = false;
        if(mPerks.containsKey("Meditations")&& mPerks.get("Meditations")!=null)
        isMeditations = mPerks.get("Meditations");
        if (isMeditations) {
            mMagicDefense *= 1.1;
            mCriticalHitChance *= 0.85;
        }
        boolean isObservant = false;
        if(mPerks.containsKey("Observant")&& mPerks.get("Observant")!=null)
        isObservant = mPerks.get("Observant");
        if (isObservant) {
